        ...
        try:
            for item in payloads:
                ...
                if existing_pcount:
                    ...
                    # job_date_dash = datetime.now().strftime("%Y-%m-%d")
                    # message_type = item.get("MessageType", "")   # ← use the item's real type, don't hardcode
                    ...
                    #     WHERE TouchTime LIKE %s AND AccountId = %s AND MessageType = %s
                    #     """,
                    #     [f"%{job_date_dash}%", AccountId, message_type]
                    # )
                    # count = count_rows[0]['Count'] if count_rows else 1
...
class PostInvoiceTable(APIView):
    ...
    back exactly what was persisted and mirrors THAT into InvoiceDtl —
    ...
    def _mirror_to_in(self, cursor, saved_row, permit_id, sno):
        """
        Upsert InvoiceDtl using the row actually persisted in CommonInvoiceDtl
        (fetched fresh) — not the original request payload.
        """
        cursor.execute(
            f"SELECT COUNT(*) FROM {self.in_table} WHERE PermitId=%s AND SNo=%s",
            [permit_id, sno]
        )
        exists = cursor.fetchone()[0] > 0

        # Only mirror columns that InvoiceDtl actually has (same set here,
        # but keeps this safe if the two schemas ever diverge slightly)
        mirror_cols = [c for c in self.allowed_columns if c in saved_row]
        ...
        if exists:
            ...
            cursor.execute(
                f"UPDATE {self.in_table} SET {set_clause} WHERE PermitId=%s AND SNo=%s",
                values
            )
        else:
    ...
    def post(self, request):
        ...
        try:
            with connections['default'].cursor() as cursor:
                for item in payloads:
                    ...
                    # Step 3: Mirror that persisted row into InvoiceDtl
                    try:
                        self._mirror_to_in(cursor, saved_row, permit_id, sno)
        ...
        response_data = {
            "Result": f"Invoice {common_action} successfully (mirrored to InvoiceDtl)",
            "Records": records,
        }
        if in_failed:
            response_data["Warning"] = (
                f"Saved to CommonInvoiceDtl but mirror to InvoiceDtl failed: {in_failed}"
            )
...
class PostItemTable(APIView):
    ...
    exactly what was persisted and mirrors THAT into ItemDtl — not the raw
    payload. Same pattern as PostInvoiceTable: guarantees the two tables can
    never drift even if Common applies any transformation during save.
    """
    ...
    # Only the columns ItemDtl actually has — CommonItemDtl minus its 15 extra columns
    mirror_columns = [
        "ItemNo", "PermitId", "MessageType", "HSCode", "Description", "DGIndicator", "Contry",
    ...
    def _mirror_to_in(self, cursor, saved_row, permit_id, item_no):
        """
        Upsert ItemDtl using the row actually persisted in CommonItemDtl
        (fetched fresh) — not the original request payload. Only columns
        that ItemDtl has are mirrored.
        """
        cursor.execute(
    ...
    def post(self, request):
        ...
        try:
            with connections['default'].cursor() as cursor:
                for item in payloads:
                    ...
                    # Step 3: Mirror that persisted row into ItemDtl
                    try:
        ...
        response_data = {
            "Result": f"Item {common_action} successfully (mirrored to ItemDtl)",
            "Records": records,
        }
        if in_failed:
            response_data["Warning"] = (
                f"Saved to CommonItemDtl but mirror to ItemDtl failed: {in_failed}"
            )
...
class PostCascTable(APIView):
    """
    Saves casc rows to CommonCASCDtl first (source of truth), then reads back
    exactly what was persisted and mirrors THAT into CASCDtl — not the raw
    payload. Same pattern as PostInvoiceTable/PostItemTable.
    """
    common_table = "CommonCASCDtl"
    in_table = "CASCDtl"

    # Full column set for CommonCASCDtl (everything except Id)
    allowed_columns = [
        "ItemNo", "ProductCode", "Quantity", "ProductUOM", "RowNo",
    ...
    # Only the columns CASCDtl actually has — CommonCASCDtl minus EndUserDes
    mirror_columns = [
        "ItemNo", "ProductCode", "Quantity", "ProductUOM", "RowNo",
        "CascCode1", "CascCode2", "CascCode3", "PermitId", "MessageType",
    ...
    def post(self, request):
        ...
        try:
            with connections['default'].cursor() as cursor:
                for item in payloads:
                    ...
                    # Step 1: Save to CommonCASCDtl (source of truth)
                    ...
                    # Step 3: Mirror that persisted row into CASCDtl
                    try:
        ...
        if in_failed:
            response_data["Warning"] = (
                f"Saved to CommonCASCDtl but mirror to CASCDtl failed: {in_failed}"
            )
