...
#05-02-26
#-------Header Table-------
class GetInnonHeaderTable(APIView):
    table = "InNonHeaderTbl"
    def get(self, request):
        data = SqlDb.execute_query(f"SELECT * FROM {self.table}")
        return Response(data)


class GetInnonHeaderByPermitId(APIView):
    table = "InNonHeaderTbl"
    def get(self, request):
            try:
...
# # Post inheader table
# class PostInnonHeaderTable(APIView):
#     table = "InNonHeaderTbl"
#     allowed_columns = {
...
#         "ExporterCompanyCode", "InwardCarrierAgentCode", "OutwardCarrierAgentCode",
#         "ConsigneeCode", "FreightForwarderCode", "ClaimantPartyCode",
...
#         "RecepilocaName", "StorageLocation", "ExhibitionSDate", "ExhibitionEDate",
...
#         "GrossReference", "DeclareIndicator", "NumberOfItems", "TotalCIFFOBValue",
#         "TotalGSTTaxAmt", "TotalExDutyAmt", "TotalCusDutyAmt", "TotalODutyAmt",
#         "TotalAmtPay", "Status", "TouchUser", "TouchTime", "PermitNumber",
#         "prmtStatus", "ReleaseLocaName", "Inhabl", "outhbl", "seastore", "Cnb",
#         "DeclarningFor", "MRDate", "MRTime", "CondColor", "TransmitId", "gstVerified"
#     }

#     # ── Columns that are genuinely nullable date/numeric fields in the DB ──
#     # Empty values for these become NULL. Everything else becomes "" instead.
#     NULLABLE_COLUMNS = {
#         "ArrivalDate", "BlanketStartDate", "ExhibitionSDate", "ExhibitionEDate",
#         "DepartureDate", "MRDate", "Refid",
#     }
...
class EditInnonHeaderByPermit(APIView):
    table = "InNonHeaderTbl"
...
class DeleteInnonPermit(APIView):
    table = "InNonHeaderTbl"
    def get(self, request):
        try:
            permit_id = request.GET.get("PermitId")
...
class InnonInvoiceTable(APIView):
    table="InNonInvoiceDtl"
    def get(self,request):
        data = SqlDb.execute_query(f"SELECT * FROM {self.table}")
        return Response(data)


class GetInnonInvoiceByInvoiceNo(APIView):
    table = "InNonInvoiceDtl"
...
class GetInnonInvoiceByEditPermitId(APIView):
    table = "InNonInvoiceDtl"
...
class GetInnonInvoiceByPermitId(APIView):
    table = "InNonInvoiceDtl"
...
class DeleteInnonInnvoice(APIView):
    table = "InNonInvoiceDtl"
    def post(self, request):
...
# class PostInnonInvoiceTable(APIView):
#     common_table = "CommonInvoiceDtl"
#     innon_table = "InNonInvoiceDtl"
#     allowed_columns = {
...
class PostInnonInvoiceTable(APIView):
    ...
    back exactly what was persisted and mirrors THAT into InNonInvoiceDtl —
    ...
    # Columns as they exist on InNonInvoiceDtl (Id is auto/PK, excluded)
    innon_columns = [
    ...
    def _mirror_to_innon(self, cursor, saved_row, permit_id, sno):
        """
        Upsert InNonInvoiceDtl using the row actually persisted in
        CommonInvoiceDtl (fetched fresh) — not the original request payload.
        """
        cursor.execute(
            f"SELECT COUNT(*) FROM {self.innon_table} WHERE PermitId=%s AND SNo=%s",
            [permit_id, sno]
        )
        exists = cursor.fetchone()[0] > 0

        # Only mirror columns that InNonInvoiceDtl actually has
        mirror_cols = [c for c in self.innon_columns if c in saved_row]
        ...
        if exists:
            ...
            cursor.execute(
                f"UPDATE {self.innon_table} SET {set_clause} WHERE PermitId=%s AND SNo=%s",
                values
            )
        else:
            ...
            cursor.execute(
                f"INSERT INTO {self.innon_table} ({', '.join(mirror_cols)}) VALUES ({placeholders})",
                values
            )
    ...
    def post(self, request):
        ...
        try:
            with connections['default'].cursor() as cursor:
                for item in payloads:
                    ...
                    # Payload is filtered against InNonInvoiceDtl's columns
                    # since that's the schema this endpoint deals with
                    columns = [k for k in self.innon_columns if k in item]
                    if not columns:
                        continue
                    ...
                    # Step 3: Mirror that persisted row into InNonInvoiceDtl
                    try:
                        self._mirror_to_innon(cursor, saved_row, permit_id, sno)
                    except Exception as mirror_err:
                        innon_failed = str(mirror_err)
        ...
        try:
            with connections['default'].cursor() as cursor:
                fetch_columns = ", ".join(self.innon_columns)
                cursor.execute(
                    f"SELECT {fetch_columns} FROM {self.common_table} "
                    f"WHERE PermitId=%s ORDER BY SNo",
                    [permit_id]
                )
                rows = cursor.fetchall()
                records = [dict(zip(self.innon_columns, row)) for row in rows]
        except Exception:
            records = []

        response_data = {
            "Result": f"Invoice {common_action} successfully (mirrored to InNonInvoiceDtl)",
            "Records": records,
        }
        if innon_failed:
            response_data["Warning"] = (
                f"Saved to CommonInvoiceDtl but mirror to InNonInvoiceDtl failed: {innon_failed}"
            )

        return Response(response_data, status=201)


class EditInnonInvoiceByInvoiceNo(APIView):
    table = "InNonInvoiceDtl"
...
class GetInnonItemTabel(APIView):
    table = "InNonItemDtl"
    def get(self, request):
        data = SqlDb.execute_query(f"SELECT * FROM {self.table}")
        return Response(data)

class GetInnonItemByItemNo(APIView):
    table = "InNonItemDtl"
...
class GetInnonItemByEditPermitId(APIView):
    table = "InNonItemDtl"
    def get(self, request):
            try:
                permit_id = request.GET.get("PermitId")
...
class DeleteInnonItem(APIView):
    table = "InNonItemDtl"
    casc_table = "INNONCASCDtl"
...
# class PostInnonItemTable(APIView):
#     table = "InNonItemDtl"
#     allowed_columns = [
#        "ItemNo", "PermitId", "MessageType", "HSCode", "Description", "DGIndicator", "Contry",
#         "Brand", "Model", "InHAWBOBL", "OutHAWBOBL", "DutiableQty", "DutiableUOM", "TotalDutiableQty",
...
#        # Maps mismatched payload keys -> real InNonItemDtl column names
#     COLUMN_ALIASES = {
...
class PostInnonItemTable(APIView):
    ...
    exactly what was persisted and mirrors THAT into InNonItemDtl — not the
    raw payload. Same pattern as PostItemTable, but InNonItemDtl renames
    3 columns (EngineCapcity->Enginecapacity, EngineCapUOM->Engineuom,
    orignaldatereg->Orginregdate) and lacks the certificate/CER columns.
    """
    common_table = "CommonItemDtl"
    in_table = "InNonItemDtl"
    ...
    # CommonItemDtl column name -> InNonItemDtl column name.
    # Every CommonItemDtl column that InNonItemDtl also has, mapped 1:1,
    # except the 3 renamed ones. Columns not listed here (EndUserDescription,
    # CerItemQty, CerItemUOM, CIFValOfCer, ManufactureCostDate, TexCat,
    # TexQuotaQty, TexQuotaUOM, CerInvNo, CerInvDate, OriginOfCer, HSCodeCer,
    # PerContent, CertificateDescription) don't exist on InNonItemDtl and
    # are simply skipped when mirroring.
    MIRROR_COLUMN_MAP = {
        ...
        "CurrentLot": "CurrentLot", "PreviousLot": "PreviousLot", "LSPValue": "LSPValue",
    ...
    def _mirror_to_in(self, cursor, saved_row, permit_id, item_no):
        """
        Upsert InNonItemDtl using the row actually persisted in
        CommonItemDtl (fetched fresh) — not the original request payload.
        Renames columns per MIRROR_COLUMN_MAP; columns InNonItemDtl doesn't
        have are dropped.
        """
        mirror_item = {
        ...
        cursor.execute(
            f"SELECT COUNT(*) FROM {self.in_table} WHERE PermitId=%s AND ItemNo=%s",
            [permit_id, item_no]
        )
        exists = cursor.fetchone()[0] > 0
        ...
        if exists:
            ...
            cursor.execute(
                f"UPDATE {self.in_table} SET {set_clause} WHERE PermitId=%s AND ItemNo=%s",
                values
            )
        else:
            placeholders = ", ".join(["%s"] * len(mirror_cols))
            ...
            cursor.execute(
                f"INSERT INTO {self.in_table} ({', '.join(mirror_cols)}) VALUES ({placeholders})",
                values
            )
    ...
    def post(self, request):
        ...
        try:
            with connections['default'].cursor() as cursor:
                for item in payloads:
                    ...
                    # Step 3: Mirror that persisted row into InNonItemDtl
                    try:
                        self._mirror_to_in(cursor, saved_row, permit_id, item_no)
                    except Exception as mirror_err:
                        in_failed = str(mirror_err)
        ...
        response_data = {
            "Result": f"Item {common_action} successfully (mirrored to InNonItemDtl)",
            "Records": records,
        }
        if in_failed:
            response_data["Warning"] = (
                f"Saved to CommonItemDtl but mirror to InNonItemDtl failed: {in_failed}"
            )
...
class EditInnonItemByItemNo(APIView):
    table = "InNonItemDtl"
...
class GetInnonCascTabel(APIView):
    table = "INNONCASCDtl"
    def get(self, request):
        data = SqlDb.execute_query(f"SELECT * FROM {self.table}")
        return Response(data)

class GetInnonCascByPermitId(APIView):
    table = "INNONCASCDtl"
    def get(self, request, permit_id):
        query = f"SELECT * FROM {self.table} WHERE PermitId = %s"
        data = SqlDb.execute_query(query, [permit_id])
...
class DeleteInnonCascByCascId(APIView):
    table = "INNONCASCDtl"
...
# class PostInnonCascTable(APIView):
#     table = "INNONCASCDtl"
#     def post(self, request):
#         payloads = request.data
#         if not isinstance(payloads, list):
...
class PostInnonCascTable(APIView):
    ...
    back exactly what was persisted and mirrors THAT into INNONCASCDtl.
    Same pattern as PostCascTable — INNONCASCDtl lacks EndUserDes.
    """
    common_table = "CommonCASCDtl"
    in_table = "INNONCASCDtl"
    ...
    def _mirror_to_in(self, cursor, saved_row):
        ...
        cursor.execute(
            f"""
            SELECT COUNT(*) FROM {self.in_table}
            WHERE ItemNo=%s AND PermitId=%s AND RowNo=%s AND CASCId=%s
            """,
            [item_no, permit_id, row_no, casc_id]
        )
        ...
        if exists:
            ...
            cursor.execute(
                f"UPDATE {self.in_table} SET {set_clause} WHERE ItemNo=%s AND PermitId=%s AND RowNo=%s AND CASCId=%s",
                values
            )
        else:
            placeholders = ", ".join(["%s"] * len(mirror_cols))
    ...
    def post(self, request):
        ...
        try:
            with connections['default'].cursor() as cursor:
                for item in payloads:
                    ...
                    try:
                        self._mirror_to_in(cursor, saved_row)
                    except Exception as mirror_err:
                        in_failed = str(mirror_err)
        ...
        if in_failed:
            response_data["Warning"] = (
                f"Saved to CommonCASCDtl but mirror to INNONCASCDtl failed: {in_failed}"
            )

        return Response(response_data, status=200)

class EditInnonCascByPermitId(APIView):
    table = "INNONCASCDtl"
    allowed_columns = {
        "ItemNo", "ProductCode", "Quantity", "ProductUOM", "RowNo",
        "CascCode1", "CascCode2", "CascCode3", "PermitId", "MessageType",
...
class GetInnonCpcTabel(APIView):
    table = "InNonCPCDtl"
    def get(self, request):
        data = SqlDb.execute_query(f"SELECT * FROM {self.table}")
        return Response(data)

class GetInnonCpcByEditPermitId(APIView):
    table ="InNonCPCDtl"
    def get(self, request):
            try:
                permit_id = request.GET.get("PermitId")
...
class GetInnonCpcByPermitId(APIView):
    table = "InNonCPCDtl"
    def get(self, request, permit_id):
        # Parameterized query to prevent SQL injection
        query = f"SELECT * FROM {self.table} WHERE PermitId = %s"
...
# class PostInnonCpcTable(APIView):
#     table = "InNonCPCDtl"
#     allowed_columns = [
...
class PostInnonCpcTable(APIView):
    table = "InnonCPCDtl"   # ← adjust to your actual mirror table name if different
    allowed_columns = [
        "PermitId", "MessageType", "RowNo", "CPCType", "ProcessingCode1",
        "ProcessingCode2", "ProcessingCode3", "TouchUser", "TouchTime"
    ]
...
class EditInnonCpcByPermitId(APIView):
    table = "InNonCPCDtl"
    def put(self, request, permit_id):
        payload = request.data
        if not payload:
...
class GetInnonContainerTabel(APIView):
    table = "InnonContainerDtl"
    def get(self, request):
        data = SqlDb.execute_query(f"SELECT * FROM {self.table}")
        return Response(data)

class GetInnonContainerByEditPermitId(APIView):
    table = "InnonContainerDtl"
    def get(self, request):
            try:
                permit_id = request.GET.get("PermitId")
...
class GetInnonContainerByPermitId(APIView):
    table = "InnonContainerDtl"
    def get(self, request, permit_id):
        # Parameterized query to prevent SQL injection
        query = f"SELECT * FROM {self.table} WHERE PermitId = %s"
...
class DeleteInnonContainer(APIView):
    table = "InnonContainerDtl"
    def post(self, request):
        permit_id = request.data.get("PermitId")
...
class PostInnonContainerTable(APIView):
    table = "InnonContainerDtl"
    allowed_columns = {
...
class EditInnonContainerByPermitId(APIView):
    table = "InnonContainerDtl"
    def put(self, request, permit_id):
        payload = request.data
        if not payload:
...
#-------------InNonImporter (Inpayment)--------------------
class GetInnonImporterTable(APIView):
    table = "InNonImporter"
    def get(self, request):
        data = SqlDb.execute_query(f"SELECT * FROM {self.table} ORDER BY Code")
        return Response(data)

class GetInnonImporterByCode(APIView):
    table = "InNonImporter"
...
class DeleteInnonImporter(APIView):
    table = "InNonImporter"
...
class PostInnonImporterTable(APIView):
    table = "InNonImporter"
...
class EditInnonImporterByCode(APIView):
    table = "InNonImporter"
    allowed_columns = {"Code", "Name", "Name1", "CRUEI", "TouchUser", "TouchTime", "Status"}
...
#---------------- Freight Forwarder ----------------#
class GetInnonFreightForwarderTable(APIView):
...
class GetInnonFreightForwarderByCode(APIView):
    table = "FreightForwarder"
...
class DeleteInnonFreightForwarder(APIView):
    table = "FreightForwarder"
...
class PostInnonFreightForwarderTable(APIView):
    table = "FreightForwarder"
    allowed_columns = {"Code", "Name", "Name1", "CRUEI", "TouchUser", "TouchTime", "Status"}
...
class EditInnonFreightForwarderByCode(APIView):
    table = "FreightForwarder"
    allowed_columns = {"Code", "Name", "Name1", "CRUEI", "TouchUser", "TouchTime", "Status"}
...
class GetInnonClaimantPartyTable(APIView):
...
class GetInnonClaimantPartyById(APIView):
    table = "InnonClaimantParty"
...
#--------------InnonExporter---------------
class GetInnonExporterTabel(APIView):
    table = "InnonExporter"
    def get(self, request):
        data = SqlDb.execute_query(f"SELECT * FROM {self.table}")
        return Response(data)

class GetInnonExporterByCode(APIView):
    table = "InnonExporter"
    def get(self, request, code):
        # Parameterized query to prevent SQL injection
        query = f"SELECT * FROM {self.table} WHERE Code = %s"
...
class DeleteInnonExporter(APIView):
    table = "InnonExporter"
    def delete(self, request, code):
        select_query = f"SELECT * FROM {self.table} WHERE Code = %s"
        existing = SqlDb.execute_query(select_query, [code])
...
class PostInnonExporterTable(APIView):
    table = "InnonExporter"
    allowed_columns = {"Code", "Name", "Name1", "CRUEI", "TouchUser", "TouchTime", "Status"}
...
class EditInnonExporterByCode(APIView):
    table = "InnonExporter"
    allowed_columns = {"Code", "Name", "Name1", "CRUEI", "TouchUser", "TouchTime", "Status"}
...
class GetInnonInwardCarrierAgent(APIView):
    table = "InnonInwardCarrierAgent"
    def get(self, request):
        data = SqlDb.execute_query(f"SELECT * FROM {self.table}")
        return Response(data)

class GetInnonInwardCarrierAgentByCode(APIView):
    table = "InnonInwardCarrierAgent"
    def get(self, request, code):
        # Parameterized query to prevent SQL injection
        query = f"SELECT * FROM {self.table} WHERE Code = %s"
...
class DeleteInnonInwardCarrierAgent(APIView):
    table = "InnonInwardCarrierAgent"
    def delete(self, request, code):
        select_query = f"SELECT * FROM {self.table} WHERE Code = %s"
        existing = SqlDb.execute_query(select_query, [code])
...
class PostInnonInwardCarrierAgentTable(APIView):
    table = "InnonInwardCarrierAgent"
    allowed_columns = {"Code", "Name", "Name1", "CRUEI", "TouchUser", "TouchTime", "Status"}
...
class EditInnonInwardCarrierAgentByCode(APIView):
    table = "InnonInwardCarrierAgent"
    allowed_columns = {"Code", "Name", "Name1", "CRUEI", "TouchUser", "TouchTime", "Status"}
...
class GetInnonOutwardCarrierAgent(APIView):
    table = "InnonOutwardCarrierAgent"
    def get(self, request):
        data = SqlDb.execute_query(f"SELECT * FROM {self.table}")
        return Response(data)

class GetInnonOutwardCarrierAgentByCode(APIView):
    table = "InnonOutwardCarrierAgent"
    def get(self, request, code):
        # Parameterized query to prevent SQL injection
        query = f"SELECT * FROM {self.table} WHERE Code = %s"
...
class DeleteInnonOutwardCarrierAgent(APIView):
    table = "InnonOutwardCarrierAgent"
    def delete(self, request, code):
        select_query = f"SELECT * FROM {self.table} WHERE Code = %s"
        existing = SqlDb.execute_query(select_query, [code])
...
class PostInnonOutwardCarrierAgentTable(APIView):
    table = "InnonOutwardCarrierAgent"
    allowed_columns = {"Code", "Name", "Name1", "CRUEI", "TouchUser", "TouchTime", "Status"}
...
class EditInnonOutwardCarrierAgentByCode(APIView):
    table = "InnonOutwardCarrierAgent"
    allowed_columns = {"Code", "Name", "Name1", "CRUEI", "TouchUser", "TouchTime", "Status"}
...
class GetInnonConsigneeTable(APIView):
    table = "InnonConsignee"
    def get(self, request):
        data = SqlDb.execute_query(f"SELECT * FROM {self.table}")
        return Response(data)


class GetInnonConsigneeByCode(APIView):
    table = "InnonConsignee"
...
class DeleteInnonConsigneeAgent(APIView):
    table = "InnonConsignee"
...
class PostInnonConsigneeTable(APIView):
    table = "InnonConsignee"
    allowed_columns = {
...
class EditInnonConsigneeByCode(APIView):
    table = "InnonConsignee"
    allowed_columns = {
        "ConsigneeCode", "ConsigneeName", "ConsigneeName1", "ConsigneeCRUEI",
        "ConsigneeAddress", "ConsigneeAddress1", "ConsigneeCity", "ConsigneeSub",
...
class GetInnonFileTable(APIView):
    table = "InNonFile"
    def get(self, request):
        data = SqlDb.execute_query(f"SELECT * FROM {self.table}")
        return Response(data)

class GetInnonFileByEditPermitId(APIView):
    table = "InNonFile"
    def get(self, request):
        try:
            permit_id = request.GET.get("PermitId")
...
class GetInnonFileByPermitId(APIView):
    table = "InNonFile"
    def get(self, request, permit_id):
        # Parameterized query to prevent SQL injection
        query = f"SELECT * FROM {self.table} WHERE PermitId = %s"
...
class PostInnonFileTable(APIView):
    table = "InNonFile"
    allowed_columns = [
...
class EditInnonFileByPermitId(APIView):
    table = "InNonFile"
    allowed_columns = {
        "Sno", "Name", "ContentType", "Data", "DocumentType",
        "InPaymentId", "TouchUser", "TouchTime",
...
class InnonItemExcelUpload(APIView):
    def post(self, request):
        ...
        try:
            with connections['default'].cursor() as cursor:
                cursor.execute(
                    "SELECT COUNT(*) FROM InNonItemDtl WHERE PermitId=%s",
                    [permit_id]
                )
        ...
        ITEM_COLUMNS = [
                ...
                "TouchUser", "TouchTime",
                "VehicleType", "Enginecapacity", "Engineuom",   # ← renamed
                "Orginregdate",                                  # ← renamed
        ...
        try:
            with connections['default'].cursor() as cursor:
                ...
                for _, row in ItemInfo.iterrows():
                    ...
                    cursor.execute(
                        "SELECT COUNT(*) FROM InNonItemDtl WHERE PermitId=%s AND ItemNo=%s",
                        [permit_id, item_no]
                    )
                    exists = cursor.fetchone()[0] > 0
                    if exists:
                        ...
                        cursor.execute(
                            f"UPDATE InNonItemDtl SET {set_clause} WHERE PermitId=%s AND ItemNo=%s",
                            update_values
                        )
                    else:
                        placeholders = ", ".join(["%s"] * len(ITEM_COLUMNS))
                        cursor.execute(
                            f"INSERT INTO InNonItemDtl ({','.join(ITEM_COLUMNS)}) VALUES ({placeholders})",
                            values
                        )
                # =========================================================
                # CASC INSERT / UPDATE
                ...
                for _, row in CascInfo.iterrows():
                    ...
                    cursor.execute(
                        """
                        SELECT COUNT(*) FROM INNONCASCDtl
                        WHERE ItemNo=%s AND PermitId=%s AND RowNo=%s AND CASCId=%s
                        """,
                    ...
                    if casc_exists:
                        ...
                        cursor.execute(
                            f"""
                            INSERT INTO INNONCASCDtl ({','.join(CASC_COLUMNS)})
        ...
        try:
            with connections['default'].cursor() as cursor:
                cursor.execute(
                    "SELECT * FROM InNonItemDtl WHERE PermitId=%s ORDER BY ItemNo",
                    [permit_id]
                )
                item_columns = [col[0] for col in cursor.description]
                items = [dict(zip(item_columns, r)) for r in cursor.fetchall()]
                cursor.execute(
                    "SELECT * FROM INNONCASCDtl WHERE PermitId=%s ORDER BY ItemNo",
                    [permit_id]
                )
...
#Edit all Item
# ===================== In-All Item Update (InNonItemDtl) =====================
class InnonAllItemUpdate(APIView):
    def post(self, request):
        try:
        ...
        try:
            ...
            with transaction.atomic():
                ...
                for permit_id in permits:
                    cursor.execute("""
                        SELECT PermitId FROM InNonHeaderTbl WHERE PermitId = %s
                    """, [permit_id])
                    ...
                    cursor.execute("""
                        INSERT INTO InNonHeaderTbl (
                            ...
                            CONSIGNEECode, InnonClaimantPartyCode, EndUserCode, Manufacturer,
...
class PrintGst(APIView):
    def get(self, request, PermitId):
        try:
            ...
            if not importer_row:
                return HttpResponse("InNonImporter not found", status=404)
...
class DownloadCcp(APIView):
    def get(self, request):
        ...
        for PermitId in data.split(','):
            ...
            p.drawString(lftcol, 590, "InnonExporter:")
...
# item instant copy from common

class SyncInnonItemFromCommon(APIView):
    # Destination column order for InNonItemDtl
    INNON_ITEM_COLUMNS = [
        "ItemNo", "PermitId", "MessageType", "HSCode", "Description", "DGIndicator", "Contry",
        ...
        "Enginecapacity", "Optioncahrge", "OptionalSumtotal", "OptionalSumExchage",
        "Engineuom", "Orginregdate",
    ]

    # Corresponding SOURCE column order from CommonItemDtl (positional match
    # to INNON_ITEM_COLUMNS above — same names except the 3 renamed ones)
    COMMON_ITEM_COLUMNS = [
        "ItemNo", "PermitId", "MessageType", "HSCode", "Description", "DGIndicator", "Contry",
        "Brand", "Model", "InHAWBOBL", "OutHAWBOBL", "DutiableQty", "DutiableUOM",
        "TotalDutiableQty", "TotalDutiableUOM", "InvoiceQuantity", "HSQty", "HSUOM",
    ...
    def post(self, request):
        ...
        innon_item_cols  = ", ".join(self.INNON_ITEM_COLUMNS)
        common_item_cols = ", ".join(self.COMMON_ITEM_COLUMNS)
        casc_cols        = ", ".join(self.CASC_COLUMNS)

        try:
            with connections['default'].cursor() as cursor:
                # ---- ITEM ----
                cursor.execute("DELETE FROM InNonItemDtl WHERE PermitId=%s", [permit_id])
                cursor.execute(
                    f"""
                    INSERT INTO InNonItemDtl ({innon_item_cols})
                    SELECT {common_item_cols}
                    FROM CommonItemDtl
                    WHERE PermitId=%s
                    """,
                    [permit_id]
                ...
                # ---- CASC ----
                cursor.execute("DELETE FROM INNONCASCDtl WHERE PermitId=%s", [permit_id])
                cursor.execute(
                    f"""
                    INSERT INTO INNONCASCDtl ({casc_cols})
                    SELECT {casc_cols}
                    FROM CommonCASCDtl
                    WHERE PermitId=%s
                    """,
                ...
                cursor.execute(
                    "SELECT * FROM InNonItemDtl WHERE PermitId=%s ORDER BY ItemNo", [permit_id]
                )
                item_columns = [c[0] for c in cursor.description]
                items = [dict(zip(item_columns, r)) for r in cursor.fetchall()]

                cursor.execute(
                    "SELECT * FROM INNONCASCDtl WHERE PermitId=%s ORDER BY ItemNo", [permit_id]
                )
        ...
        return Response({
            "Result": "InNon-tables synced from Common tables successfully",
            "item": items,
            "casc": casc,
        }, status=200)


# postinnonitemwithcasc
class PostInnonItemWithCascTable(APIView):
    """
    Single-call replacement for PostInnonItemTable + PostInnonCascTable.
    ...
    Flow, all inside one DB transaction:
      ...
      3. Mirror that row -> InNonItemDtl (renaming EngineCapcity/EngineCapUOM/
         orignaldatereg to Enginecapacity/Engineuom/Orginregdate, dropping
         columns InNonItemDtl doesn't have)
      4. For each casc row: upsert -> CommonCASCDtl, read back, mirror -> INNONCASCDtl
    """
    common_item_table = "CommonItemDtl"
    in_item_table = "InNonItemDtl"
    common_casc_table = "CommonCASCDtl"
    in_casc_table = "INNONCASCDtl"
    ...
    # CommonItemDtl column -> InNonItemDtl column (renamed 3 + skip columns
    # InNonItemDtl doesn't have: EndUserDescription, Cer*/Tex* certificate fields)
    ...
    def _mirror_item(self, cursor, saved_row, permit_id, item_no):
        """
        Upsert InNonItemDtl using the row actually persisted in CommonItemDtl,
        applying ITEM_MIRROR_COLUMN_MAP for the 3 renamed columns and dropping
        columns InNonItemDtl doesn't have.
        """
        mirror_item = {
            dest_col: saved_row.get(src_col)
    ...
    def post(self, request):
        ...
        try:
            with connections['default'].cursor() as cursor:
                for item in payloads:
                    ...
                    # ===== ITEM: upsert Common -> read back -> mirror to InNonItemDtl =====
                    ...
                    # ===== CASC: same pattern, per row, mirror to INNONCASCDtl =====
        ...
        response_data = {
            "Result": (
                ...
                f"(mirrored to InNonItemDtl/INNONCASCDtl)"
            ),
            "Records": records,
        }
        warnings = []
        if item_mirror_failed:
            warnings.append(f"Item mirror to InNonItemDtl failed: {item_mirror_failed}")
        if casc_mirror_failed:
            warnings.append(f"CASC mirror to INNONCASCDtl failed: {casc_mirror_failed}")
        if warnings:
