...
#05-02-26
#-------Header Table-------
class GetOutHeaderTable(APIView):
    table = "OutHeaderTbl"
...
class GetOutHeaderByPermitId(APIView):
    table = "OutHeaderTbl"
    def get(self, request):
            try:
...
# class PostOutHeaderTable(APIView):
#     table = "OutHeaderTbl"
#     allowed_columns = {
...
#         "License", "COType", "Entryyear", "GSPDonorCountry",
#         "CerDetailtype1", "CerDetailCopies1", "CerDetailtype2", "CerDetailCopies2",
#         "PerCommon", "CurrencyCode", "AddCerDtl", "TransDtl", "Recipient",
#         "DeclarantCompanyCode", "ExporterCompanyCode", "Inwardcarriercode",
#         "OutwardCarrierAgentCode", "FreightForwarderCode", "ImporterCompanyCode",
#         "InwardCarrierAgentCode", "CONSIGNEECode", "EndUserCode", "Manufacturer",
#         "ArrivalDate", "ArrivalTime", "LoadingPortCode", "VoyageNumber", "VesselName",
...
#         "DepartureDate", "DepartureTime", "DischargePort", "FinalDestinationCountry",
#         "OutVoyageNumber", "OutVesselName", "OutOceanBillofLadingNo",
#         "VesselType", "VesselNetRegTon", "VesselNationality", "TowingVesselID",
#         "TowingVesselName", "NextPort", "LastPort", "OutConveyanceRefNo",
#         "OutTransportId", "OutFlightNO", "OutAircraftRegNo", "OutMasterAirwayBill",
#         "TotalOuterPack", "TotalOuterPackUOM", "TotalGrossWeight", "TotalGrossWeightUOM",
#         "GrossReference", "TradeRemarks", "InternalRemarks",
#         "DeclareIndicator", "NumberOfItems", "TotalCIFFOBValue",
#         "TotalGSTTaxAmt", "TotalExDutyAmt", "TotalCusDutyAmt", "TotalODutyAmt",
#         "TotalAmtPay", "Status", "TouchUser", "TouchTime", "PermitNumber",
#         "prmtStatus", "ResLoaName", "RepLocName", "RecepitLocName",
#         "outHAWB", "INHAWB", "CertificateNumber", "Defrentprinting", "Cnb",
#         "DeclarningFor", "MRDate", "MRTime", "CondColor", "TransmitId"
#     }

#     NULLABLE_COLUMNS = {
#         "ArrivalDate", "ArrivalTime", "BlanketStartDate",
#         "DepartureDate", "DepartureTime", "MRDate", "Refid",
#     }
        ...
        try:
            for item in payloads:
                ...
                if common_header_row:
                    ...
                    # PermitCount-la row illati, insert pannunga (tracking-ku)
...
class EditOutHeaderByPermit(APIView):
    table = "OutHeaderTbl"
...
class GetOutInvoiceByInvoiceNo(APIView):
    table = "OutInvoiceDtl"
...
class GetOutInvoiceByEditPermitId(APIView):
    table = "OutInvoiceDtl"
...
class GetOutInvoiceByPermitId(APIView):
    table = "OutInvoiceDtl"
...
class DeleteOutInnvoice(APIView):
    table = "OutInvoiceDtl"
    def post(self, request):
...
# class PostOutInvoiceTable(APIView):
#     table = "OutInvoiceDtl"
#     allowed_columns = {
#         "PermitId","SNo","InvoiceNo","InvoiceDate","TermType",
#         "AdValoremIndicator","PreDutyRateIndicator",
#         "SupplierImporterRelationship","SupplierCode","ExportPartyCode",
...
class PostOutInvoiceTable(APIView):
    """
    Saves invoice to CommonInvoiceDtl first (source of truth), then reads
    back exactly what was persisted and mirrors THAT into OutInvoiceDtl —
    not the raw payload. This guarantees the two tables can never drift
    even if Common applies any transformation during save.
    """
    common_table = "CommonInvoiceDtl"
    out_table = "OutInvoiceDtl"

    # Columns as they exist on OutInvoiceDtl — note ExportPartyCode
    # (not ImportPartyCode) is the Out-side equivalent.
    out_columns = [
        ...
        "MessageType", "PermitId", "TouchUser", "TouchTime",
    ]

    def _upsert_common(self, cursor, columns, item, permit_id, sno):
        """Upsert into CommonInvoiceDtl. Returns 'inserted' or 'updated'."""
        cursor.execute(
            f"SELECT COUNT(*) FROM {self.common_table} WHERE PermitId=%s AND SNo=%s",
            [permit_id, sno]
        )
        exists = cursor.fetchone()[0] > 0

        if exists:
            update_cols = [c for c in columns if c not in ("PermitId", "SNo")]
            set_clause = ", ".join([f"{c}=%s" for c in update_cols])
            values = [item.get(c) for c in update_cols] + [permit_id, sno]
            cursor.execute(
                f"UPDATE {self.common_table} SET {set_clause} WHERE PermitId=%s AND SNo=%s",
                values
            )
            return "updated"
        else:
    ...
    def _mirror_to_out(self, cursor, saved_row, permit_id, sno):
        """
        Upsert OutInvoiceDtl using the row actually persisted in
        CommonInvoiceDtl (fetched fresh) — not the original request payload.
        """
        cursor.execute(
            f"SELECT COUNT(*) FROM {self.out_table} WHERE PermitId=%s AND SNo=%s",
            [permit_id, sno]
        )
        exists = cursor.fetchone()[0] > 0

        # Only mirror columns that OutInvoiceDtl actually has
        mirror_cols = [c for c in self.out_columns if c in saved_row]

        if exists:
            update_cols = [c for c in mirror_cols if c not in ("PermitId", "SNo")]
            set_clause = ", ".join([f"{c}=%s" for c in update_cols])
            values = [saved_row.get(c) for c in update_cols] + [permit_id, sno]
            cursor.execute(
                f"UPDATE {self.out_table} SET {set_clause} WHERE PermitId=%s AND SNo=%s",
                values
            )
        else:
    ...
    def post(self, request):
        ...
        try:
            with connections['default'].cursor() as cursor:
                for item in payloads:
                    ...
                    # Common's ImportPartyCode == Out's ExportPartyCode for
                    # this record; the frontend sends both, so pick whichever
                    # is present and normalize into ImportPartyCode for Common.
                    if "ImportPartyCode" not in item and "ExportPartyCode" in item:
                        item["ImportPartyCode"] = item["ExportPartyCode"]
                    ...
                    columns = [k for k in common_allowed if k in item]
                    if not columns:
                        continue
                    ...
                    if not permit_id or not sno:
                        return Response(
                            {"error": "PermitId and SNo are required"}, status=400
                        )

                    # Step 1: Save to CommonInvoiceDtl (source of truth)
                    common_action = self._upsert_common(
                        cursor, columns, item, permit_id, sno
                    )

                    # Step 2: Read back EXACTLY what was persisted
                    fetch_cols = ", ".join(columns)
                    cursor.execute(
                        f"SELECT {fetch_cols} FROM {self.common_table} "
                        f"WHERE PermitId=%s AND SNo=%s",
                        [permit_id, sno]
                    )
                    row = cursor.fetchone()
                    saved_row = dict(zip(columns, row)) if row else item
                    # Out side keys off ExportPartyCode, so carry the value over
                    if "ImportPartyCode" in saved_row:
                        saved_row["ExportPartyCode"] = saved_row["ImportPartyCode"]

                    # Step 3: Mirror that persisted row into OutInvoiceDtl
                    try:
                        self._mirror_to_out(cursor, saved_row, permit_id, sno)
        ...
        # Return the Common records (what the frontend table displays)
        try:
            with connections['default'].cursor() as cursor:
                fetch_columns = ", ".join(self.out_columns)
                cursor.execute(
                    f"SELECT {fetch_columns.replace('ExportPartyCode', 'ImportPartyCode AS ExportPartyCode')} "
                    f"FROM {self.common_table} WHERE PermitId=%s ORDER BY SNo",
                    [permit_id]
                )
                rows = cursor.fetchall()
                records = [dict(zip(self.out_columns, row)) for row in rows]
        except Exception:
            records = []

        response_data = {
            "Result": f"Invoice {common_action} successfully (mirrored to OutInvoiceDtl)",
            "Records": records,
        }
        if out_failed:
            response_data["Warning"] = (
                f"Saved to CommonInvoiceDtl but mirror to OutInvoiceDtl failed: {out_failed}"
            )

        return Response(response_data, status=201)


class EditOutInvoiceByInvoiceNo(APIView):
    table = "OutInvoiceDtl"
...
class GetOutItemTabel(APIView):
    table = "OutItemDtl"
    def get(self, request):
        data = SqlDb.execute_query(f"SELECT * FROM {self.table}")
        return Response(data)

class GetOutItemByItemNo(APIView):
    table = "OutItemDtl"
...
class GetOutItemByEditPermitId(APIView):
    table = "OutItemDtl"
    def get(self, request):
            try:
                permit_id = request.GET.get("PermitId")
...
class DeleteOutItem(APIView):
    table = "OutItemDtl"
    casc_table = "OutCASCDtl"
...
# class PostOutItemTable(APIView):
#     table = "OutItemDtl"
#     allowed_columns = [
...
#         "InvoiceNo", "ChkUnitPrice", "UnitPrice", "UnitPriceCurrency", "ExchangeRate", "SumExchangeRate",
...
#         "CerItemQty", "CerItemUOM", "CIFValOfCer", "ManufactureCostDate", "TexCat",
#         "TexQuotaQty", "TexQuotaUOM", "CerInvNo", "CerInvDate", "OriginOfCer",
#         "HSCodeCer", "PerContent", "CertificateDescription",
#     ]
#        # Maps mismatched payload keys -> real OutItemDtl column names
#     COLUMN_ALIASES = {
...
class PostOutItemTable(APIView):
    ...
    exactly what was persisted and mirrors THAT into OutItemDtl — not the
    raw payload. Same pattern as PostInnonItemTable: OutItemDtl renames
    3 columns (EngineCapcity->Enginecapacity, EngineCapUOM->Engineuom,
    orignaldatereg->Orginregdate) and lacks LSPValue.
    """
    common_table = "CommonItemDtl"
    out_table = "OutItemDtl"
    ...
    # CommonItemDtl column name -> OutItemDtl column name.
    # LSPValue is skipped: OutItemDtl has no such column.
    MIRROR_COLUMN_MAP = {
        "ItemNo": "ItemNo", "PermitId": "PermitId", "MessageType": "MessageType",
        "HSCode": "HSCode", "Description": "Description", "DGIndicator": "DGIndicator",
        "Contry": "Contry", "EndUserDescription": "EndUserDescription",
        ...
        "AlcoholPer": "AlcoholPer", "InvoiceNo": "InvoiceNo",
        ...
        # "LSPValue" intentionally omitted — no such column on OutItemDtl
        ...
        "CerItemQty": "CerItemQty", "CerItemUOM": "CerItemUOM", "CIFValOfCer": "CIFValOfCer",
        "ManufactureCostDate": "ManufactureCostDate", "TexCat": "TexCat",
        "TexQuotaQty": "TexQuotaQty", "TexQuotaUOM": "TexQuotaUOM",
        "CerInvNo": "CerInvNo", "CerInvDate": "CerInvDate", "OriginOfCer": "OriginOfCer",
        "HSCodeCer": "HSCodeCer", "PerContent": "PerContent",
        "CertificateDescription": "CertificateDescription",
        "TouchUser": "TouchUser", "TouchTime": "TouchTime", "VehicleType": "VehicleType",
        ...
        "EngineCapUOM": "EngineCapUOM",
        "orignaldatereg": "orignaldatereg",
    }
    ...
    def _mirror_to_out(self, cursor, saved_row, permit_id, item_no):
        """
        Upsert OutItemDtl using the row actually persisted in CommonItemDtl
    ...
    def post(self, request):
        ...
        try:
            with connections['default'].cursor() as cursor:
                for item in payloads:
                    ...
                    # Step 3: Mirror that persisted row into OutItemDtl
                    try:
        ...
        response_data = {
            "Result": f"Item {common_action} successfully (mirrored to OutItemDtl)",
            "Records": records,
        }
        if out_failed:
            response_data["Warning"] = (
                f"Saved to CommonItemDtl but mirror to OutItemDtl failed: {out_failed}"
            )

        return Response(response_data, status=201)


class EditOutItemByItemNo(APIView):
    table = "OutItemDtl"
...
class GetOutCascTabel(APIView):
    table = "OutCASCDtl"
    def get(self, request):
        data = SqlDb.execute_query(f"SELECT * FROM {self.table}")
        return Response(data)

class GetOutCascByPermitId(APIView):
    table = "OutCASCDtl"
    def get(self, request, permit_id):
        query = f"SELECT * FROM {self.table} WHERE PermitId = %s"
        data = SqlDb.execute_query(query, [permit_id])
...
class DeleteOutCascByCascId(APIView):
    table = "OutCASCDtl"
...
# class PostOutCascTable(APIView):
#     table = "OutCASCDtl"
#     def post(self, request):
#         payloads = request.data
#         if not isinstance(payloads, list):
...
class PostOutCascTable(APIView):
    ...
    back exactly what was persisted and mirrors THAT into OutCASCDtl.
    Same pattern as PostInnonCascTable — OutCASCDtl lacks EndUserDes.
    """
    common_table = "CommonCASCDtl"
    out_table = "OutCASCDtl"
    ...
    def post(self, request):
        ...
        if out_failed:
            response_data["Warning"] = (
                f"Saved to CommonCASCDtl but mirror to OutCASCDtl failed: {out_failed}"
            )

        return Response(response_data, status=200)

class EditOutCascByPermitId(APIView):
    table = "OutCASCDtl"
    allowed_columns = {
        "ItemNo", "ProductCode", "Quantity", "ProductUOM", "RowNo",
        "CascCode1", "CascCode2", "CascCode3", "PermitId", "MessageType",
...
class GetOutCpcTabel(APIView):
    table = "OutCPCDtl"
    def get(self, request):
        data = SqlDb.execute_query(f"SELECT * FROM {self.table}")
        return Response(data)

class GetOutCpcByEditPermitId(APIView):
    table ="OutCPCDtl"
    def get(self, request):
            try:
                permit_id = request.GET.get("PermitId")
...
class GetOutCpcByPermitId(APIView):
    table = "OutCPCDtl"
    def get(self, request, permit_id):
        # Parameterized query to prevent SQL injection
        query = f"SELECT * FROM {self.table} WHERE PermitId = %s"
...
class EditOutCpcByPermitId(APIView):
    table = "OutCPCDtl"
    def put(self, request, permit_id):
        payload = request.data
        if not payload:
...
class GetOutContainerTabel(APIView):
    table = "OutContainerDtl"
    def get(self, request):
        data = SqlDb.execute_query(f"SELECT * FROM {self.table}")
        return Response(data)

class GetOutContainerByEditPermitId(APIView):
    table = "OutContainerDtl"
    def get(self, request):
            try:
                permit_id = request.GET.get("PermitId")
...
class GetOutContainerByPermitId(APIView):
    table = "OutContainerDtl"
    def get(self, request, permit_id):
        # Parameterized query to prevent SQL injection
        query = f"SELECT * FROM {self.table} WHERE PermitId = %s"
...
class DeleteOutContainer(APIView):
    table = "OutContainerDtl"
    def post(self, request):
        permit_id = request.data.get("PermitId")
...
class PostOutContainerTable(APIView):
    table = "OutContainerDtl"
    allowed_columns = {
...
class EditOutContainerByPermitId(APIView):
    table = "OutContainerDtl"
    def put(self, request, permit_id):
        payload = request.data
        if not payload:
...
#-------------OutContainerDtl (Outpayment)--------------------
class GetOutImporterTable(APIView):
    table = "OutImporter"
    def get(self, request):
        data = SqlDb.execute_query(f"SELECT * FROM {self.table} ORDER BY Code")
        return Response(data)

class GetOutImporterByCode(APIView):
    table = "OutImporter"
...
class DeleteOutImporter(APIView):
    table = "OutImporter"
...
class PostOutImporterTable(APIView):
    table = "OutImporter"
...
class EditOutImporterByCode(APIView):
    table = "OutImporter"
    allowed_columns = {"Code", "Name", "Name1", "CRUEI", "TouchUser", "TouchTime", "Status"}
...
#---------------- Freight Forwarder ----------------#
class GetOutFreightForwarderTable(APIView):
...
class GetOutFreightForwarderByCode(APIView):
    table = "FreightForwarder"
...
class DeleteOutFreightForwarder(APIView):
    table = "FreightForwarder"
...
class PostOutFreightForwarderTable(APIView):
    table = "FreightForwarder"
    allowed_columns = {"Code", "Name", "Name1", "CRUEI", "TouchUser", "TouchTime", "Status"}
...
class EditOutFreightForwarderByCode(APIView):
    table = "FreightForwarder"
    allowed_columns = {"Code", "Name", "Name1", "CRUEI", "TouchUser", "TouchTime", "Status"}
...
class GetOutClaimantPartyTable(APIView):
...
class GetOutClaimantPartyById(APIView):
    table = "InnonClaimantParty"
...
#--------------OutExporter---------------
class GetOutExporterTable(APIView):
    table = "OutExporter"
    def get(self, request):
        data = SqlDb.execute_query(f"SELECT * FROM {self.table}")
        return Response(data)

class GetOutExporterByCode(APIView):
    table = "OutExporter"
    def get(self, request, code):
        # Parameterized query to prevent SQL injection
        query = f"SELECT * FROM {self.table} WHERE Code = %s"
...
class GetOutInwardCarrierAgent(APIView):
    table = "InwardCarrierAgent"
    def get(self, request):
        data = SqlDb.execute_query(f"SELECT * FROM {self.table}")
        return Response(data)

class GetOutInwardCarrierAgentByCode(APIView):
    table = "InwardCarrierAgent"
    def get(self, request, code):
        # Parameterized query to prevent SQL injection
        query = f"SELECT * FROM {self.table} WHERE Code = %s"
...
class DeleteOutInwardCarrierAgent(APIView):
    table = "InwardCarrierAgent"
    def delete(self, request, code):
        select_query = f"SELECT * FROM {self.table} WHERE Code = %s"
        existing = SqlDb.execute_query(select_query, [code])
...
class PostOutInwardCarrierAgentTable(APIView):
    table = "InwardCarrierAgent"
    allowed_columns = {"Code", "Name", "Name1", "CRUEI", "TouchUser", "TouchTime", "Status"}
...
class EditOutInwardCarrierAgentByCode(APIView):
    table = "InwardCarrierAgent"
    allowed_columns = {"Code", "Name", "Name1", "CRUEI", "TouchUser", "TouchTime", "Status"}
...
class GetOutOutwardCarrierAgent(APIView):
    table = "OutwardCarrierAgent"
    def get(self, request):
        data = SqlDb.execute_query(f"SELECT * FROM {self.table}")
        return Response(data)

class GetOutOutwardCarrierAgentByCode(APIView):
    table = "OutwardCarrierAgent"
    def get(self, request, code):
        # Parameterized query to prevent SQL injection
        query = f"SELECT * FROM {self.table} WHERE Code = %s"
...
class DeleteOutOutwardCarrierAgent(APIView):
    table = "OutwardCarrierAgent"
    def delete(self, request, code):
        select_query = f"SELECT * FROM {self.table} WHERE Code = %s"
        existing = SqlDb.execute_query(select_query, [code])
...
class PostOutOutwardCarrierAgentTable(APIView):
    table = "OutwardCarrierAgent"
    allowed_columns = {"Code", "Name", "Name1", "CRUEI", "TouchUser", "TouchTime", "Status"}
...
class EditOutOutwardCarrierAgentByCode(APIView):
    table = "OutwardCarrierAgent"
    allowed_columns = {"Code", "Name", "Name1", "CRUEI", "TouchUser", "TouchTime", "Status"}
...
class GetOutConsigneeTable(APIView):
    table = "OutConsignee"
    def get(self, request):
        data = SqlDb.execute_query(f"SELECT * FROM {self.table}")
        return Response(data)
...
class GetOutConsigneeByCode(APIView):
    table = "OutConsignee"
...
class DeleteOutConsignee(APIView):
    table = "OutConsignee"
...
class PostOutConsigneeTable(APIView):
    table = "OutConsignee"
    allowed_columns = {
...
class EditOutConsigneeByCode(APIView):
    table = "OutConsignee"
    allowed_columns = {
        "ConsigneeCode", "ConsigneeName", "ConsigneeName1", "ConsigneeCRUEI",
        "ConsigneeAddress", "ConsigneeAddress1", "ConsigneeCity", "ConsigneeSub",
...
class GetOutManufacturerTable(APIView):
...
class GetOutManufacturerByCode(APIView):
    table = "OutManufacturer"
...
class GetOutFileTable(APIView):
    table = "OutFile"
    def get(self, request):
        data = SqlDb.execute_query(f"SELECT * FROM {self.table}")
        return Response(data)

class GetOutFileByEditPermitId(APIView):
    table = "OutFile"
    def get(self, request):
        try:
            permit_id = request.GET.get("PermitId")
...
class GetOutFileByPermitId(APIView):
    table = "OutFile"
    def get(self, request, permit_id):
        # Parameterized query to prevent SQL injection
        query = f"SELECT * FROM {self.table} WHERE PermitId = %s"
...
class PostOutFileTable(APIView):
    table = "OutFile"
    allowed_columns = [
...
class EditOutFileByPermitId(APIView):
    table = "OutFile"
    allowed_columns = {
        "Sno", "Name", "ContentType", "Data", "DocumentType",
        "InPaymentId", "TouchUser", "TouchTime",
...
class OutItemExcelUpload(APIView):
    def post(self, request):
        ...
        try:
            with connections['default'].cursor() as cursor:
                cursor.execute(
                    "SELECT COUNT(*) FROM OutItemDtl WHERE PermitId=%s",
                    [permit_id]
                )
        ...
        try:
            with connections['default'].cursor() as cursor:
                ...
                for _, row in ItemInfo.iterrows():
                    ...
                    cursor.execute(
                        "SELECT COUNT(*) FROM OutItemDtl WHERE PermitId=%s AND ItemNo=%s",
                        [permit_id, item_no]
                    )
                    exists = cursor.fetchone()[0] > 0
                    if exists:
                        ...
                        cursor.execute(
                            f"UPDATE OutItemDtl SET {set_clause} WHERE PermitId=%s AND ItemNo=%s",
                            update_values
                        )
                    else:
                        placeholders = ", ".join(["%s"] * len(ITEM_COLUMNS))
                        cursor.execute(
                            f"INSERT INTO OutItemDtl ({','.join(ITEM_COLUMNS)}) VALUES ({placeholders})",
                            values
                        )
                ...
                for _, row in CascInfo.iterrows():
                    ...
                    if casc_exists:
                        ...
                        cursor.execute(
                            f"""
                            INSERT INTO OutCASCDtl ({','.join(CASC_COLUMNS)})
        ...
        try:
            with connections['default'].cursor() as cursor:
                cursor.execute(
                    "SELECT * FROM OutItemDtl WHERE PermitId=%s ORDER BY ItemNo",
                    [permit_id]
                )
                item_columns = [col[0] for col in cursor.description]
                items = [dict(zip(item_columns, r)) for r in cursor.fetchall()]
                cursor.execute(
                    "SELECT * FROM OutCASCDtl WHERE PermitId=%s ORDER BY ItemNo",
                    [permit_id]
                )
...
#Edit all Item
# ===================== In-All Item Update (OutItemDtl) =====================
# class OutAllItemUpdate(APIView):
#     def post(self, request):
#         try:
...
class OutAllItemUpdate(APIView):
    def post(self, request):
        try:
            ...
            for item in Item:
                # ---- FIXED: column names now match the real OutItemDtl schema ----
                # - "LSPValue" removed: no such column exists in OutItemDtl
                # - Enginecapacity -> EngineCapcity (real column name in this table)
                # - Engineuom -> EngineCapUOM (real column name in this table)
                # - Orginregdate -> orignaldatereg (real column name in this table)
                # - InvoiceNo and VehicleType kept as-is: they DO exist in OutItemDtl
                SqlDb.execute_query("""
                    UPDATE OutItemDtl SET
                        MessageType=%s, HSCode=%s, Description=%s,
                        DGIndicator=%s, Contry=%s, Brand=%s, Model=%s,
...
class DeleteOutHawbl(APIView):
    def delete(self, request, permit_id):
        try:
            SqlDb.execute_query(
                "UPDATE OutItemDtl SET InHAWBOBL=''WHERE PermitId=%s",
                (permit_id,)
            )
            return Response(
...
class DeleteInHawbl(APIView):
    def delete(self, request, permit_id):
        try:
            SqlDb.execute_query(
                "UPDATE OutItemDtl SET InHAWBOBL='', OutHAWBOBL='' WHERE PermitId=%s",
                (permit_id,)
            )
            return Response(
                {"message": f"HAWBOBL cleared for all items in PermitId {permit_id}"},
        ...
        try:
            ...
            with transaction.atomic():
                ...
                for permit_id in permits:
                    cursor.execute("""
                        SELECT PermitId FROM OutHeaderTbl WHERE PermitId = %s
                    """, [permit_id])
...
class PrintGst(APIView):
    def get(self, request, PermitId):
        try:
            ...
            if not importer_row:
                return HttpResponse("OutContainerDtl not found", status=404)
...
# item instant copy from common (OUT)
class SyncOutItemFromCommon(APIView):

    # Destination column order for OutItemDtl (actual schema, from your DB)
    OUT_ITEM_COLUMNS = [
        "ItemNo", "PermitId", "MessageType", "HSCode", "Description", "DGIndicator", "Contry",
        "EndUserDescription", "Brand", "Model", "InHAWBOBL", "OutHAWBOBL", "DutiableQty", "DutiableUOM",
        "TotalDutiableQty", "TotalDutiableUOM", "InvoiceQuantity", "HSQty", "HSUOM", "AlcoholPer",
        ...
        "CustomsDutyUOM", "CustomsDutyAmount", "OtherTaxRate", "OtherTaxUOM", "OtherTaxAmount",
        "CurrentLot", "PreviousLot", "Making", "ShippingMarks1", "ShippingMarks2",
        "ShippingMarks3", "ShippingMarks4", "CerItemQty", "CerItemUOM", "CIFValOfCer",
        "ManufactureCostDate", "TexCat", "TexQuotaQty", "TexQuotaUOM", "CerInvNo", "CerInvDate",
        "OriginOfCer", "HSCodeCer", "PerContent", "CertificateDescription",
        "TouchUser", "TouchTime", "VehicleType", "OptionalChrgeUOM",
        "EngineCapcity", "Optioncahrge", "OptionalSumtotal", "OptionalSumExchage",
        "EngineCapUOM", "orignaldatereg",
    ]

    # CommonItemDtl's actual columns (used only to know what exists there).
    # Columns in OUT_ITEM_COLUMNS that AREN'T in this set become NULL in the SELECT.
    COMMON_ITEM_COLUMNS_SET = {
        "ItemNo", "PermitId", "MessageType", "HSCode", "Description", "DGIndicator", "Contry",
        "Brand", "Model", "InHAWBOBL", "OutHAWBOBL", "DutiableQty", "DutiableUOM",
        "TotalDutiableQty", "TotalDutiableUOM", "InvoiceQuantity", "HSQty", "HSUOM",
        "AlcoholPer", "InvoiceNo", "ChkUnitPrice", "UnitPrice", "UnitPriceCurrency",
        "ExchangeRate", "SumExchangeRate", "TotalLineAmount", "InvoiceCharges", "CIFFOB",
        "OPQty", "OPUOM", "IPQty", "IPUOM", "InPqty", "InPUOM", "ImPQty", "ImPUOM",
        "PreferentialCode", "GSTRate", "GSTUOM", "GSTAmount", "ExciseDutyRate", "ExciseDutyUOM",
        "ExciseDutyAmount", "CustomsDutyRate", "CustomsDutyUOM", "CustomsDutyAmount",
        "OtherTaxRate", "OtherTaxUOM", "OtherTaxAmount", "CurrentLot", "PreviousLot",
        "LSPValue", "Making", "ShippingMarks1", "ShippingMarks2", "ShippingMarks3",
        "ShippingMarks4", "TouchUser", "TouchTime", "VehicleType", "OptionalChrgeUOM",
        "EngineCapcity", "Optioncahrge", "OptionalSumtotal", "OptionalSumExchage",
        "EngineCapUOM", "orignaldatereg",
    }
    ...
    def post(self, request):
        ...
        out_item_cols = ", ".join(self.OUT_ITEM_COLUMNS)

        # Build the SELECT expression list by NAME, not by position.
        # Any OUT column that doesn't exist in CommonItemDtl becomes NULL.
        select_exprs = [
            col if col in self.COMMON_ITEM_COLUMNS_SET else "NULL"
            for col in self.OUT_ITEM_COLUMNS
        ]
        common_select_cols = ", ".join(select_exprs)
        ...
        try:
            with connections['default'].cursor() as cursor:
                # ---- ITEM ----
                cursor.execute("DELETE FROM OutItemDtl WHERE PermitId=%s", [permit_id])
                cursor.execute(
                    f"""
                    INSERT INTO OutItemDtl ({out_item_cols})
                    SELECT {common_select_cols}
                    FROM CommonItemDtl
                    WHERE PermitId=%s
                    """,
                    [permit_id]
                ...
                # ---- CASC ----
                cursor.execute("DELETE FROM OutCASCDtl WHERE PermitId=%s", [permit_id])
                cursor.execute(
                    f"""
                    INSERT INTO OutCASCDtl ({casc_cols})
                    SELECT {casc_cols}
                    FROM CommonCASCDtl
                    WHERE PermitId=%s
                    """,
                ...
                cursor.execute(
                    "SELECT * FROM OutItemDtl WHERE PermitId=%s ORDER BY ItemNo", [permit_id]
                )
                item_columns = [c[0] for c in cursor.description]
                items = [dict(zip(item_columns, r)) for r in cursor.fetchall()]

                cursor.execute(
                    "SELECT * FROM OutCASCDtl WHERE PermitId=%s ORDER BY ItemNo", [permit_id]
                )
        ...
        return Response({
            "Result": "Out-tables synced from Common tables successfully",
            "item": items,
            "casc": casc,
        }, status=200)
...
class PostOutItemWithCascTable(APIView):
    """
    Single-call replacement for PostOutItemTable + PostOutCascTable.
    ...
    Flow, all inside one DB transaction:
      ...
      3. Mirror that row -> OutItemDtl (no renaming needed — OutItemDtl uses
         the same Engine/date column names as CommonItemDtl; LSPValue is
         dropped since OutItemDtl has no such column)
      4. For each casc row: upsert -> CommonCASCDtl, read back, mirror ->
         OutCASCDtl (including EndUserDes, which OutCASCDtl has)
    """
    common_item_table = "CommonItemDtl"
    out_item_table = "OutItemDtl"
    common_casc_table = "CommonCASCDtl"
    out_casc_table = "OutCASCDtl"
    ...
    # CommonItemDtl column -> OutItemDtl column.
    # LSPValue intentionally omitted — no such column on OutItemDtl.
    # Everything else (including EndUserDescription and the Cer*/Tex* fields)
    # exists on OutItemDtl under the SAME name — no renaming needed here,
    # unlike InNonItemDtl.
    ...
    # OutCASCDtl has EndUserDes, so mirror it too (unlike INNONCASCDtl)
    casc_mirror_columns = [
        "ItemNo", "ProductCode", "Quantity", "ProductUOM", "RowNo",
        "CascCode1", "CascCode2", "CascCode3", "PermitId", "MessageType",
        "TouchUser", "TouchTime", "CASCId", "EndUserDes"
    ...
    def _mirror_item(self, cursor, saved_row, permit_id, item_no):
        """
        Upsert OutItemDtl using the row actually persisted in CommonItemDtl,
        applying ITEM_MIRROR_COLUMN_MAP (drops LSPValue; everything else
        maps 1:1 by name).
        """
        mirror_item = {
            dest_col: saved_row.get(src_col)
        ...
        cursor.execute(
            f"SELECT COUNT(*) FROM {self.out_item_table} WHERE PermitId=%s AND ItemNo=%s",
            [permit_id, item_no]
        )
        exists = cursor.fetchone()[0] > 0
        ...
        if exists:
            ...
            values = [mirror_item.get(c) for c in update_cols] + [permit_id, item_no]
            cursor.execute(
                f"UPDATE {self.out_item_table} SET {set_clause} WHERE PermitId=%s AND ItemNo=%s",
                values
            )
        else:
            placeholders = ", ".join(["%s"] * len(mirror_cols))
            values = [mirror_item.get(c) for c in mirror_cols]
            cursor.execute(
                f"INSERT INTO {self.out_item_table} ({', '.join(mirror_cols)}) VALUES ({placeholders})",
                values
            )
    ...
    def _mirror_casc(self, cursor, saved_row):
        ...
        cursor.execute(
            f"""
            SELECT COUNT(*) FROM {self.out_casc_table}
            WHERE ItemNo=%s AND PermitId=%s AND RowNo=%s AND CASCId=%s
            """,
            [item_no, permit_id, row_no, casc_id]
        )
        ...
        if exists:
            ...
            values = [saved_row.get(c) for c in update_cols] + [item_no, permit_id, row_no, casc_id]
            cursor.execute(
                f"UPDATE {self.out_casc_table} SET {set_clause} "
            ...
            values = [saved_row.get(c) for c in mirror_cols]
            cursor.execute(
                f"INSERT INTO {self.out_casc_table} ({', '.join(mirror_cols)}) VALUES ({placeholders})",
                values
            )
    ...
    def post(self, request):
        ...
        try:
            with connections['default'].cursor() as cursor:
                for item in payloads:
                    ...
                    # ===== ITEM: upsert Common -> read back -> mirror to OutItemDtl =====
                    ...
                    # ===== CASC: same pattern, per row, mirror to OutCASCDtl =====
        ...
        response_data = {
            "Result": (
                ...
                f"(mirrored to OutItemDtl/OutCASCDtl)"
            ),
            "Records": records,
        }
        warnings = []
        if item_mirror_failed:
            warnings.append(f"Item mirror to OutItemDtl failed: {item_mirror_failed}")
        if casc_mirror_failed:
            warnings.append(f"CASC mirror to OutCASCDtl failed: {casc_mirror_failed}")
        if warnings:
