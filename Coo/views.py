...
#05-02-26
#-------Header Table-------
class GetCooHeaderTable(APIView):
    table = "COHeaderTbl"
...
class GetCooHeaderByPermitId(APIView):
    table = "COHeaderTbl"
    def get(self, request):
            try:
...
# class PostCooHeaderTable(APIView):
#     table = "COHeaderTbl"
#     allowed_columns = {
# "Refid",
# "JobId",
...
# Post coo header table
class PostCooHeaderTable(APIView):
    table = "COHeaderTbl"
    allowed_columns = {
        "Refid","JobId","MSGId","PermitId","TradeNetMailboxID","MessageType","ApplicationType","PreviousPermitNo","OutwardTransportMode",
        "ReferenceDocuments","COType","CerDtlType1","CerDtlCopy1","CerDtlType2","CerDtlCopy2","CurrencyCode","AdditionalCer",
        "TransportDtl","PerferenceContent","DeclarantCompanyCode","ExporterCompanyCode","OutwardCarrierAgentCode","FreightForwarderCode",
        "CONSIGNEECode","Manufacturer","DepartureDate","DischargePort","FinalDestinationCountry","OutVoyageNumber","OutVesselName",
        "OutConveyanceRefNo","OutTransportId","OutFlightNO","OutAircraftRegNo","TotalOuterPack","TotalOuterPackUOM",
        "TotalGrossWeight","TotalGrossWeightUOM","DeclareIndicator","NumberOfItems","InternalRemarks","TradeRemarks","Status",
        "TouchUser","TouchTime","prmtStatus","PermitNumber","EntryYear","Percomwealth","Gpsdonorcountry","Additionalrecieptant",
        "GrossReference","DeclarningFor","CertificateNumber","MRDate","MRTime","TransmitId"
        }
...
class EditCooHeaderByPermit(APIView):
    table = "COHeaderTbl"
...
class GetCooItemTable(APIView):
    table = "COItemDtl"
    def get(self, request):
        data = SqlDb.execute_query(f"SELECT * FROM {self.table}")
        return Response(data)

class GetCooItemByItemNo(APIView):
    table = "COItemDtl"
...
class GetCooItemByEditPermitId(APIView):
    table = "COItemDtl"
    def get(self, request):
            try:
                permit_id = request.GET.get("PermitId")
...
class DeleteCooItem(APIView):
    table = "COItemDtl"
    # casc_table = "COCasctbl"
...
class GetCooCascTabel(APIView):
    table = "COCasctbl"
    def get(self, request):
        data = SqlDb.execute_query(f"SELECT * FROM {self.table}")
        return Response(data)

class GetCooCascByPermitId(APIView):
    table = "COCasctbl"
...
class DeleteCooCascByCascId(APIView):
    table = "COCasctbl"
...
class PostCooCascTable(APIView):
    table = "COCasctbl"
    def post(self, request):
        payloads = request.data
        if not isinstance(payloads, list):
...
class EditCooCascByPermitId(APIView):
    table = "COCasctbl"
    allowed_columns = {
...
#---------------- Freight Forwarder ----------------#
class GetCooFreightTable(APIView):
    table = "COFreightForwarder"
    def get(self, request):
        data = SqlDb.execute_query(f"SELECT * FROM {self.table} ORDER BY Code")
        return Response(data)


class GetCooFreightByCode(APIView):
    table = "COFreightForwarder"
...
class DeleteCooFreight(APIView):
    table = "COFreightForwarder"
...
class PostCooFreightTable(APIView):
    table = "COFreightForwarder"
    allowed_columns = {"Code", "Name", "Name1", "CRUEI", "TouchUser", "TouchTime", "Status"}
...
class EditCooFreightByCode(APIView):
    table = "COFreightForwarder"
    allowed_columns = {"Code", "Name", "Name1", "CRUEI", "TouchUser", "TouchTime", "Status"}
...
#--------------CooExporter---------------
class GetCooExporterTable(APIView):
    table = "COExporter"
    def get(self, request):
        data = SqlDb.execute_query(f"SELECT * FROM {self.table}")
        return Response(data)

class GetCooExporterByCode(APIView):
    table = "COExporter"
    def get(self, request, code):
        # Parameterized query to prevent SQL injection
        query = f"SELECT * FROM {self.table} WHERE Code = %s"
...
class DeleteCooExporter(APIView):
    table = "COExporter"
    def delete(self, request, code):
        select_query = f"SELECT * FROM {self.table} WHERE Code = %s"
        existing = SqlDb.execute_query(select_query, [code])
...
class PostCooExporterTable(APIView):
    table = "COExporter"
    allowed_columns = {"Code", "Name", "Name1", "CRUEI", "Address", "Address1", "Address2", "TouchUser", "TouchTime", "Status"}
...
class EditCooExporterByCode(APIView):
    table = "COExporter"
    allowed_columns = {"Code", "Name", "Name1", "CRUEI", "TouchUser", "TouchTime", "Status"}
...
class GetCOOutwardCarrierAgentCarrierAgent(APIView):
    table = "COOutwardCarrierAgent"
    def get(self, request):
        data = SqlDb.execute_query(f"SELECT * FROM {self.table}")
        return Response(data)

class GetCOOutwardCarrierAgentCarrierAgentByCode(APIView):
    table = "COOutwardCarrierAgent"
    def get(self, request, code):
        # Parameterized query to prevent SQL injection
        query = f"SELECT * FROM {self.table} WHERE Code = %s"
...
class DeleteCOOutwardCarrierAgentCarrierAgent(APIView):
    table = "COOutwardCarrierAgent"
    def delete(self, request, code):
        select_query = f"SELECT * FROM {self.table} WHERE Code = %s"
        existing = SqlDb.execute_query(select_query, [code])
...
class PostCOOutwardCarrierAgentCarrierAgentTable(APIView):
    table = "COOutwardCarrierAgent"
    allowed_columns = {"Code", "Name", "Name1", "CRUEI", "TouchUser", "TouchTime", "Status"}
...
class EditCOOutwardCarrierAgentCarrierAgentByCode(APIView):
    table = "COOutwardCarrierAgent"
    allowed_columns = {"Code", "Name", "Name1", "CRUEI", "TouchUser", "TouchTime", "Status"}
...
class GetCooConsigneeTable(APIView):
    table = "COConsignee"
    def get(self, request):
        data = SqlDb.execute_query(f"SELECT * FROM {self.table}")
        return Response(data)
...
class GetCooConsigneeByCode(APIView):
    table = "COConsignee"
...
class DeleteCooConsignee(APIView):
    table = "COConsignee"
...
class PostCooConsigneeTable(APIView):
    table = "COConsignee"
    allowed_columns = {
...
class EditCooConsigneeByCode(APIView):
    table = "COConsignee"
    allowed_columns = {
        "ConsigneeCode", "ConsigneeName", "ConsigneeName1", "ConsigneeCRUEI",
        "ConsigneeAddress", "ConsigneeAddress1", "ConsigneeCity", "ConsigneeSub",
...
class GetCooManufacturerTable(APIView):
...
class GetCooManufacturerByCode(APIView):
    table = "COManufacturer"
...
class DeleteCooManufacturerByCode(APIView):
    table = "COManufacturer"
...
class PostCooManufacturerTable(APIView):
    table = "COManufacturer"
    allowed_columns = {
...
class EditCooManufacturerByCode(APIView):
    table = "COManufacturer"
    allowed_columns = {
        "ManufacturerCode", "ManufacturerName", "ManufacturerName1",
        "ManufacturerCRUEI", "ManufacturerAddress", "ManufacturerAddress1",
...
class GetCooFileTable(APIView):
    table = "COFileUpload"
    def get(self, request):
        data = SqlDb.execute_query(f"SELECT * FROM {self.table}")
        return Response(data)

class GetCooFileByEditPermitId(APIView):
    table = "COFileUpload"
    def get(self, request):
        try:
            permit_id = request.GET.get("PermitId")
...
class GetCooFileByPermitId(APIView):
    table = "COFileUpload"
    def get(self, request, permit_id):
        # Parameterized query to prevent SQL injection
        query = f"SELECT * FROM {self.table} WHERE PermitId = %s"
...
class PostCooFileTable(APIView):
    table = "COFileUpload"
    allowed_columns = [
...
class EditCooFileByPermitId(APIView):
    table = "COFileUpload"
    allowed_columns = {
        "Sno", "Name", "ContentType", "Data", "DocumentType",
        "InPaymentId", "TouchUser", "TouchTime",
...
#             with connections['default'].cursor() as cursor:
#                 cursor.execute(
#                     "SELECT COUNT(*) FROM COItemDtl WHERE PermitId=%s",
#                     [permit_id]
#                 )
...
#                     ]
#                     cursor.execute(
#                         "SELECT COUNT(*) FROM COItemDtl WHERE PermitId=%s AND ItemNo=%s",
#                         [permit_id, item_no]
#                     )
#                     exists = cursor.fetchone()[0] > 0
#                     if exists:
...
#                         cursor.execute(
#                             f"UPDATE COItemDtl SET {set_clause} WHERE PermitId=%s AND ItemNo=%s",
#                             update_values
#                         )
#                     else:
#                         placeholders = ", ".join(["%s"] * len(ITEM_COLUMNS))
#                         cursor.execute(
#                             f"INSERT INTO COItemDtl ({','.join(ITEM_COLUMNS)}) VALUES ({placeholders})",
#                             values
#                         )
...
#                         cursor.execute(
#                             f"""
#                             INSERT INTO COCasctbl ({','.join(CASC_COLUMNS)})
...
#             with connections['default'].cursor() as cursor:
#                 cursor.execute(
#                     "SELECT * FROM COItemDtl WHERE PermitId=%s ORDER BY ItemNo",
#                     [permit_id]
#                 )
#                 item_columns = [col[0] for col in cursor.description]
#                 items = [dict(zip(item_columns, r)) for r in cursor.fetchall()]
#                 cursor.execute(
#                     "SELECT * FROM COCasctbl WHERE PermitId=%s ORDER BY ItemNo",
#                     [permit_id]
#                 )
...
class CooItemExcelUpload(APIView):
    def post(self, request):
        xlsx_file = request.FILES.get("file")
        if not xlsx_file:
            return Response({"error": "No file provided"}, status=400)
        ...
        # ---- Real COItemDtl columns only (Id excluded — auto PK) ----
        ITEM_COLUMNS = [
            "ItemNo", "PermitId", "MessageType",
            "HSCode", "Description", "Contry",
            "UnitPrice", "UnitPriceCurrency", "ExchangeRate", "SumExchangeRate",
            "TotalLineAmount", "CIFFOB",
            "InvoiceQty", "HSQTY", "HSUOM",
            "ShippingMark",
            "CerItemQty", "CerItemUOM", "ManfCostDate",
            "TextileCat", "TextileQuotaQty", "TextileQuotaQtyUOM",
            "ItemValue", "InvoiceNumber", "InvoiceDate",
            "HSOnCer", "OriginCriterion", "PerOrgainCRI", "CertificateDes",
            "Touch_user", "TouchTime", "Hawblno",
        ]

        # Only aliases actually needed for the trimmed field set
        ITEM_EXCEL_MAP = {
            "CountryofOrigin": "Contry",
            "ItemCurrency":    "UnitPriceCurrency",
            "InvoiceQuantity": "InvoiceQty",
            ...
            with connections['default'].cursor() as cursor:
                ...
                for _, row in ItemInfo.iterrows():
                    ...
                    values = [
                        ...
                        to_float(mapped.get("TotalLineAmount")),
                        to_float(mapped.get("CIFFOB")),
                        to_float(mapped.get("InvoiceQty")),
                        to_float(mapped.get("HSQTY")),
                        mapped.get("HSUOM", ""),
                        mapped.get("ShippingMark", ""),
                        to_float(mapped.get("CerItemQty")),
                        mapped.get("CerItemUOM", ""),
                        excel_to_date_str(mapped.get("ManfCostDate")),
                        mapped.get("TextileCat", ""),
                        to_float(mapped.get("TextileQuotaQty")),
                        mapped.get("TextileQuotaQtyUOM", ""),
                        to_float(mapped.get("ItemValue")),
                        mapped.get("InvoiceNumber", ""),
                        excel_to_date_str(mapped.get("InvoiceDate")),
                        mapped.get("HSOnCer", ""),
                        mapped.get("OriginCriterion", ""),
                        mapped.get("PerOrgainCRI", ""),
                        mapped.get("CertificateDes", ""),
                        user_name, touch_time,
                        mapped.get("Hawblno", ""),
                    ]
...
# ===================== In-All Item Update (COItemDtl) =====================
# ===================== In-All Item Update (COItemDtl) =====================
class CooAllItemUpdate(APIView):
    def post(self, request):
        try:
            Item = request.data.get("Item")
            PermitId = request.data.get("PermitId")
            ...
            for item in Item:
                # ---- Columns now match the real COItemDtl schema ----
                SqlDb.execute_query("""
                    UPDATE COItemDtl SET
                        MessageType=%s, HSCode=%s, Description=%s, Contry=%s,
                        UnitPrice=%s, UnitPriceCurrency=%s, ExchangeRate=%s,
                        SumExchangeRate=%s, TotalLineAmount=%s, CIFFOB=%s,
                        InvoiceQty=%s, HSQTY=%s, HSUOM=%s, ShippingMark=%s,
                        CerItemQty=%s, CerItemUOM=%s,
                        TextileCat=%s, TextileQuotaQty=%s, TextileQuotaQtyUOM=%s,
                        ItemValue=%s, InvoiceNumber=%s,
                        HSOnCer=%s, OriginCriterion=%s, PerOrgainCRI=%s,
                        CertificateDes=%s, Touch_user=%s, TouchTime=%s,
                        Hawblno=%s
                    WHERE ItemNo=%s AND PermitId=%s
                """, [
                    item.get("MessageType"),
                    ...
                    item.get("TotalLineAmount"),
                    item.get("CIFFOB"),
                    item.get("InvoiceQuantity") or item.get("InvoiceQty"),
                    item.get("HSQty") or item.get("HSQTY"),
                    item.get("HSUOM"),
                    item.get("ShippingMark") or item.get("ShippingMarks1"),
                    item.get("CerItemQty"),
                    item.get("CerItemUOM"),
                    # item.get("ManfCostDate"),
                    item.get("TexCat") or item.get("TextileCat"),
                    item.get("TexQuotaQty") or item.get("TextileQuotaQty"),
                    item.get("TexQuotaUOM") or item.get("TextileQuotaQtyUOM"),
                    item.get("ItemValue"),
                    item.get("CerInvNo") or item.get("InvoiceNumber"),
                    # item.get("InvoiceDate"),
                    item.get("HSCodeCer") or item.get("HSOnCer"),
                    item.get("OriginOfCer") or item.get("OriginCriterion"),
                    item.get("PerContent") or item.get("PerOrgainCRI"),
                    item.get("CertificateDescription") or item.get("CertificateDes"),
                    item.get("TouchUser") or item.get("Touch_user"),
                    item.get("TouchTime"),
                    item.get("InHAWBOBL") or item.get("Hawblno"),
                    item.get("ItemNo"),
                    PermitId,
                ])
...
class DeleteTransHawbl(APIView):
    def delete(self, request, permit_id):
        try:
            SqlDb.execute_query(
                "UPDATE COItemDtl SET InHAWBOBL=''WHERE PermitId=%s",
                (permit_id,)
            )
            return Response(
        ...
        try:
            ...
            with transaction.atomic():
                ...
                for permit_id in permits:
                    cursor.execute("""
                        SELECT PermitId FROM COHeaderTbl WHERE PermitId = %s
                    """, [permit_id])
                    ...
                    cursor.execute("""
                        INSERT INTO COHeaderTbl (
                            ...
                            CONSIGNEECode, InnonClaimantPartyCode, EndUserCode, COManufacturer,
...
class DownloadCcp(APIView):
    def get(self, request):
        ...
        for PermitId in data.split(','):
            ...
            p.drawString(lftcol, 590, "COExporter:")
                ...
                if 700 <= itemy <= 820:
                    ...
                    for itd in ItemDtls:
                        if float(itd.get('OtherTaxAmount') or 0) != 0.0:
                            ...
                            itemy -= 10
                            break
                    p.drawString(snox, itemy, "COManufacturer'S NAME ")
                    itemy -= 10
    ...
    def get(self, request, permit_id):
        try:
            ...
            for item in ItemDtls:
                ...
                # ── COManufacturer name from matching invoice ────────────────
                for inv in InvoiceDtls:
                    if inv.get('InvoiceNo') == item.get('InvoiceNo'):
                        manfs = SqlDb.execute_query(
                            "SELECT * FROM CommonSupplierManufacturerPart WHERE Code = %s",
...
# Item Instantly copy from CommonItemDtl table (Certificate of Origin)

class SyncCooItemFromCommon(APIView):
    ...
    COMMON_ITEM_COLUMNS = [
        ...
        "TexCat", "TexQuotaQty", "TexQuotaUOM", "CIFValOfCer", "CerInvNo",
        "CerInvDate", "HSCodeCer", "OriginOfCer", "PerContent",
        "CertificateDescription", "TouchUser", "TouchTime", "InHAWBOBL",
    ]

    # Same positions — but the real column names on COItemDtl (target)
    COO_ITEM_COLUMNS = [
        "ItemNo", "PermitId", "MessageType", "HSCode", "Description", "Contry",
        "UnitPrice", "UnitPriceCurrency", "ExchangeRate", "SumExchangeRate",
        "TotalLineAmount", "CIFFOB", "InvoiceQty", "HSQTY", "HSUOM",
        "ShippingMark", "CerItemQty", "CerItemUOM", "ManfCostDate",
        "TextileCat", "TextileQuotaQty", "TextileQuotaQtyUOM", "ItemValue", "InvoiceNumber",
        "InvoiceDate", "HSOnCer", "OriginCriterion", "PerOrgainCRI",
        "CertificateDes", "Touch_user", "TouchTime", "Hawblno",
    ]
    ...
    def post(self, request):
        ...
        coo_cols = ", ".join(self.COO_ITEM_COLUMNS)
        common_cols = ", ".join(self.COMMON_ITEM_COLUMNS)

        try:
            with connections['default'].cursor() as cursor:
                # ---- ITEM ---- (no CASC table exists for COO)
                cursor.execute("DELETE FROM COItemDtl WHERE PermitId=%s", [permit_id])
                cursor.execute(
                    f"""
                    INSERT INTO COItemDtl ({coo_cols})
                    SELECT {common_cols}
                    FROM CommonItemDtl
                    WHERE PermitId=%s
                    """,
                ...
                cursor.execute(
                    "SELECT * FROM COItemDtl WHERE PermitId=%s ORDER BY ItemNo", [permit_id]
                )
        ...
        return Response({
            "Result": "COO item table synced from Common table successfully",
            "item": items,
        }, status=200)